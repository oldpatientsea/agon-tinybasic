...
Licence: 'my TinyBasic is free to use. If you want to conform to the letter of the law, 
you can attach to any copies this notice: "TinyBasic interpreter Copyright 1976 Itty 
Bitty Computers, used by permission."' http://www.ittybittycomputers.com/IttyBitty/TinyBasic/ 
...
/* do not define FileType if you don't intend to use filehandling */
#define FileHandle         UINT8
#define IoFileClose(fh)    mos_fclose(fh)
#define InFileChar(fh)     CfileRead(fh)
#define OutFileChar(fh,ch) mos_fputc(fh, ch) 
...
/* #define OutFileChar(fi,ch) fi->putch(ch) */
 ...
 char CfileRead(FileHandle fh) {   /* C file reader, returns '\0' on eof */
   int chn = mos_fgetc(fh);
  if (chn == EOF) return '\0';
...
FileHandle inFile = 0; /* from option '-i' or user menu/button */
FileHandle oFile = 0;  /* from option '-o' or user menu/button */
...
void Ouch(char ch) {                         /* output char to stdout */
    ...
    if (ch>=' ') OutFileChar(ch, oFile);
    else if (ch == '\r') OutFileChar('\r',oFile);}
  if (ch=='\r') {
...
char Inch(void) {          /* read input character from stdin or file */
  ...
  if (NeedsEcho) { /* alternative input may need this */
	if (ch == '\r') OutLn();
		else ScreenChar(ch);
  }	  
      ...
      case 39:
        ...
        while (true) {               /* read input line characters... */
          ...
          if (ch=='\r') {
            ...
            if (InLend>InLine) {
				InLend--;
				continue; /* ZDS needs this; gcc assumes this. Why? */
				/* assume console already <-- WHAT DOES THIS MEAN?*/
			}
            ...
            InLend = InLine;
            OutLn();                /* also start a new input line */
            break;}
...
void StartTinyBasic(char* ILtext) {
  ...
  Core[BScode] = 127; /*TODO appears this code is sent not 8	??? */ // 8; /* backspace */
...
int main(int argc, char* argv[]) {
  ...
  for (nx=1; nx<argc; nx++) {         /* look for command-line args.. */
      ...
      tmpFile = mos_fopen(argv[nx],fa_read);
      if (tmpFile != 0) if (fseek(tmpFile,0,SEEK_END)==0) {
      ...
      oFile = mos_fopen(argv[nx],fa_write);
    else if (strcmp(argv[nx],"-i")==0 && ++nx<argc)     /* input file */
      inFile = mos_fopen(argv[nx],fa_read);
    else if (inFile==0)  /* default (unadorned) is also input file */
      inFile = mos_fopen(argv[nx],fa_read);}             /* ignore other args */
  if (inFile==0) inFile = mos_fopen(DefaultInputFile,fa_read);
  if (oFile==0) oFile = mos_fopen(DefaultOutputFile,fa_write);
